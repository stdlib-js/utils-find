...
function find( arr, options, clbk ) { // eslint-disable-line stdlib/no-redeclare
	...
	if ( !isCollection( arr ) && !isString( arr ) ) {
		throw new TypeError( format( '1Tp2u', arr ) );
	}
	len = arr.length;
	...
	if ( !isFunction( cb ) ) {
		throw new TypeError( format( '1Tp2b', cb ) );
	}
	if ( !isObject( opts ) ) {
		throw new TypeError( format( '1Tp2V', opts ) );
	}
	if ( hasOwnProp( opts, 'k' ) ) {
		k = opts.k;
		if ( !isInteger( k ) ) {
			throw new TypeError( format( '1Tp8M', 'k', k ) );
		}
	} else {
		k = len;
	}
	if ( hasOwnProp( opts, 'returns' ) ) {
		...
		if ( !isString( ret ) || indexOf( RETURNS, ret ) === -1 ) {
			throw new TypeError( format( '1Tp4S', 'returns', RETURNS.join( '", "' ), ret ) );
		}
		if ( ret === 'values' ) {
			mode = 1;
		} else if ( ret === '*' ) {
