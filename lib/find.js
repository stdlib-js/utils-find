...
function find( arr, options, clbk ) { // eslint-disable-line stdlib/no-redeclare
	...
	if ( !isCollection( arr ) && !isString( arr ) ) {
		throw new TypeError( format( '0jl36', arr ) );
	}
	len = arr.length;
	...
	if ( !isFunction( cb ) ) {
		throw new TypeError( format( '0jl2n', cb ) );
	}
	if ( !isObject( opts ) ) {
		throw new TypeError( format( '0jl2h', opts ) );
	}
	if ( hasOwnProp( opts, 'k' ) ) {
		k = opts.k;
		if ( !isInteger( k ) ) {
			throw new TypeError( format( '0jl8e', 'k', k ) );
		}
	} else {
		k = len;
	}
	if ( hasOwnProp( opts, 'returns' ) ) {
		...
		if ( !isString( ret ) || indexOf( RETURNS, ret ) === -1 ) {
			throw new TypeError( format( '0jl3t', 'returns', RETURNS.join( '", "' ), ret ) );
		}
		if ( ret === 'values' ) {
			mode = 1;
		} else if ( ret === '*' ) {
